...
function Timer:update(dt)
    for tag, v in pairs(self.timers) do
        if v.status == "play" then 
            ...
            elseif v.type == "script" then
                if coroutine.status(v.coroutine) == "dead" then v.after(); self.timers[tag] = nil end
    ...
    for k, v in pairs(self.sinwaves) do 
        v.t = v.t + dt
        v.value = v.value + (v.amplitude * math.sin(v.t * v.speed))
    end
end
...
function Timer:script(action, a, b)
    local tag, after
    if     type(a) == "nil"      and type(b) == "nil"    then after, tag = function() end, _uid() 
    elseif type(a) == "string"   and type(b) == "nil"    then after, tag = function() end, a      
    elseif type(a) == "function" and type(b) == "nil"    then after, tag = a      , _uid() 
    ...
    local _error, _message = coroutine.resume(
        self.timers[tag].coroutine, 
        function(time, coroutine_after_tag)
            if type(time) == "number" then 
                self.timers[tag].coroutine_after_tag = self:after(time, function()
                    local _error, _message = coroutine.resume(self.timers[tag].coroutine) 
                    assert(_error, _message) 
                end, coroutine_after_tag)
                coroutine.yield()
...
function Timer:always(action, a, b, c) return self:during(math.huge, action, a, b, c) end
...
function Timer:after(tag) self.timers[tag].after(); self:remove(tag) end
function Timer:pause(tag)
    if self.timers[tag].type == "script" then 
        if self.timers[self.timers[tag].coroutine_after_tag] then self.timers[self.timers[tag].coroutine_after_tag].status = "pause" end 
    end
    self.timers[tag].status = "pause" 
end
function Timer:play(tag)
    if self.timers[tag].type == "script" then 
        if self.timers[self.timers[tag].coroutine_after_tag] then self.timers[self.timers[tag].coroutine_after_tag].status = "play" end 
    end
...
function Timer:remove(tag)
    ...
    if self.timers[tag].type == "script" then 
        if self.timers[self.timers[tag].coroutine_after_tag] then self.timers[self.timers[tag].coroutine_after_tag] = nil end 
    end
    self.timers[tag] = nil     
end
function Timer:destroy() self.timers = {} end

function Timer:lerp(a, b, x) return a + (b - a) * x end
function Timer:smooth(a, b, speed, dt) return a + (b - a) * (1.0 - math.exp(-speed * dt)) end

function Timer:spring() 

end 

function Timer:sinwave(value, speed, amplitude)  
    table.insert(self.sinwaves,
        {
            value     = value,
            speed     = speed or 1,
            amplitude = amplitude or 1,
            t         = 0,
            function set_speed(self, speed) self.speed = speed end,
            function set_amplitude(self, amplitude) self.amplitude = amplitude end
        }
    )
end 
