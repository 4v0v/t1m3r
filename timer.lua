...
function Timer:during(time, action, a, b)
    local tag, after
    if     type(a) == "nil"      and type(b) == "nil"    then after, tag = function() end, _uid() 
    elseif type(a) == "string"   and type(b) == "nil"    then after, tag = function() end, a      
    elseif type(a) == "function" and type(b) == "nil"    then after, tag = a      , _uid() 
    ...
    local _no_error, _message = coroutine.resume(self.timers[tag].coroutine, 
        function(time, after_tag)
            if type(time) == "number" then 
                self.timers[tag].script_state = "after"
            ...
            elseif type(time) == "string" or time == nil then
                self.timers[tag].script_state = "wait"
                self.timers[tag].resume_tag = time or ""
                coroutine.yield()
            end
        end
    )
...
function Timer:get_resume_tag(tag) return self.timers[tag].resume_tag end
